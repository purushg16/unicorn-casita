...
const useUserCartStore = create<UserCartStore & UserCartActions>()(
  persist(
    (set) => ({
      products: [],
      ...
      addProduct: (
        ...
        set((store) => ({
          products:
            store.products.findIndex((p) => p.productId === productId) === -1 ||
            // if there is no product
            store.products.find((p) => p.productId === productId)
              ?.attrValueId !== // if there is a product with diff attribute
              attrValue._id!
              ? [
                  ...store.products,
                  {
    ...
    {
      name: "user-cart-store", // unique name
